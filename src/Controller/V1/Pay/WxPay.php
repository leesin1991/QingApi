...
class WxPay extends Controller
{
    ...
    protected function wxPay() {
        ...
        if ($post['total'] && $post['type'] && $post['product_id']) {
            ...
            $beforePayCheckRes = $this->_createPayOrder($user_id, 2, $type, $post['product_id'],$post['total']);
            if($beforePayCheckRes){
                $bizcontent = $beforePayCheckRes;
            }else{
                return ['status' => false, 'errno' => '-1', 'errmsg' => "系统繁忙，请稍后再试！"];
            }   
//            print_r($bizcontent);die;
            $return = $this->app->getWechatOrder($bizcontent);  
            return ['status' => true, 'errno' => '0', 'data' => $return];
        } else {
